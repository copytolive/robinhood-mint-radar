...
class RadarScanner:
    ...
    def build_status(self,tip,safe_block,from_block,to_block,started_at):
        now=int(time.time());self._maybe_maintain(now);launches=self.db.launches_map();rows=self.db.recent_collections(now-3600);candidates=[];filtered=0
