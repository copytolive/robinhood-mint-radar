...
class LiveRadarScanner(BaseRadarScanner):
    ...
    def __init__(self,*args,**kwargs):
        ...
        # Discovery keeps the robust retrying RPC. Candidate enrichment has a
        # bounded latency budget: missing data becomes REVIEW/UNAVAILABLE rather
        # than freezing the live cursor for minutes.
        ...
        def cached_ownership(address,total_supply=None):
            ...
            value=self.enrich_explorer.ownership(address,total_supply)
    ...
    def _e_supports(self,address,interface_id):
        ...
        if not sel:return None
        try:
            data=sel+interface_id[2:].rjust(64,'0');out=self.enrich_rpc.eth_call(address,data)
            return bool(int(out,16)) if out and out!='0x' else False
        except Exception:return None

    def contract_snapshot(self,address):
        """Fetch independent contract facts concurrently with bounded latency."""
        ...
        def get_code():
            try:return self.enrich_rpc.code(address)
                ...
                self._diag('CONTRACT_CHECK','GET_CODE_FAILED',exc);return None

        with ThreadPoolExecutor(max_workers=10) as pool:
            futures={
                'code':pool.submit(get_code),
                'ver':pool.submit(self.enrich_explorer.verification,address),
                'total':pool.submit(self._e_uint,address,'totalSupply'),
                'minted':pool.submit(self._e_uint,address,'totalMinted'),
                'max':pool.submit(self._e_uint,address,'maxSupply'),
                'price_wei':pool.submit(self._e_uint,address,'mintPriceWei'),
                'price':pool.submit(self._e_uint,address,'mintPrice'),
                'name':pool.submit(self._e_string,address,'name'),
                'symbol':pool.submit(self._e_string,address,'symbol'),
                'owner':pool.submit(self._e_address,address,'owner'),
                'erc721':pool.submit(self._e_supports,address,'0x80ac58cd'),
                'erc1155':pool.submit(self._e_supports,address,'0xd9b67a26'),
            }
            r={k:f.result() for k,f in futures.items()}

        ver=r['ver'] or {'verified':None,'source_text':'','review_risks':['VERIFICATION_UNAVAILABLE'],'hard_risks':[],'capabilities':[]}
        total=r['total'] if r['total'] is not None else r['minted']
        price=r['price_wei'] if r['price_wei'] is not None else r['price']
        cname=ver.get('contract_name') or ''
        is_hoodsea='hoodseanft' in cname.lower()
        snap_name=r['name'];snap_ticker=r['symbol']
        if is_hoodsea and self._selectors.get('info'):
            try:
                info=self.enrich_rpc.eth_call(address,self._selectors['info'])
                snap_name=snap_name or decode_string_word(info,0);snap_ticker=snap_ticker or decode_string_word(info,1)
                if price is None:price=data_word(info,8)
            except Exception:pass
        safety=evaluate_safety(ver,owner_address=r['owner'])
        code=r['code']
        if code in ('0x','0x0'):
            safety['state']='REJECT';safety['hard_risks']=sorted(set(safety.get('hard_risks',[])+['NO_CONTRACT_CODE']))
        elif code is None:
            # Unavailable code is not equivalent to no code. Fail closed as review.
        ...
        value={'safety':safety,'supply':{'total_supply':total,'max_supply':r['max']},'mint_price_wei':price,'name':snap_name,'ticker':snap_ticker,'is_hoodsea':is_hoodsea,'contract_name':cname,'source_text':ver.get('source_text') or '','interfaces':{'erc721':r['erc721'],'erc1155':r['erc1155']}}
        self._snapshot_cache[key]=(now,value)
        return value

    def _prewarm_enrichment(self):
        """Warm current candidate snapshots/ownership concurrently before scoring."""
        now=int(time.time());rows=self.db.recent_collections(now-3600)[:config.MAX_CANDIDATES*3]
        # Prioritize newest collections, but cap one wave so a pathological hour
        # cannot turn a single live cycle into minutes. Remaining rows are retried
        # on following warm daemon cycles; unresolved data always fails closed.
        rows=rows[:max(config.MAX_CANDIDATES,8)]
        def warm(row):
            snap=self.contract_snapshot(row['collection'])
            self.explorer.ownership(row['collection'],snap['supply'].get('total_supply'))
        if rows:
            with ThreadPoolExecutor(max_workers=min(8,len(rows))) as pool:
                list(pool.map(warm,rows))

    def _scan_range_or_raise(self,from_block,to_block):
        before=len(self.diag);added=super().scan_logs(from_block,to_block)
        failures=[d for d in self.diag[before:] if d.get('reason') in _CRITICAL_LOG_FAILURES]
        if failures:
            reasons=','.join(sorted({d.get('reason') for d in failures}));raise RPCError(f'CRITICAL_LOG_SCAN_FAILED:{from_block}-{to_block}:{reasons}')
        return added
    ...
    def _lag_metrics(self,safe_block,processed_to):
        ...
        blocks=max(0,int(safe_block)-int(processed_to));safe_ts=self.block_time(int(safe_block));cursor_ts=self.block_time(int(processed_to));seconds=max(0,int(safe_ts)-int(cursor_ts));return blocks,seconds

    def _lag_is_ready(self,blocks,seconds):
        return blocks is not None and seconds is not None and blocks<=config.MAX_READY_LAG_BLOCKS and seconds<=config.MAX_READY_LAG_SECONDS
