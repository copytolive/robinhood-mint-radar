...
class RadarDB:
    ...
    def _ensure_col(self,table,name,decl):
        if name not in self._columns(table):
            self.conn.execute(f'ALTER TABLE {table} ADD COLUMN {name} {decl}')

    def _migrate(self):
        for name,decl in [
          ('payment_token','TEXT'),('payment_amount','TEXT'),('source','TEXT'),
          ('order_hash','TEXT'),('bundle_size','INTEGER DEFAULT 1')
        ]: self._ensure_col('market_events',name,decl)
        for name,decl in [
          ('predicted_score','REAL'),('predicted_probability','REAL')
        ]: self._ensure_col('outcomes',name,decl)
    ...
    def add_market_sale(self,x):
        self.conn.execute('''INSERT OR IGNORE INTO market_events(
        event_id,block_number,block_time,tx_hash,log_index,collection,event_type,token_id,price_wei,seller,buyer,raw_json,
        payment_token,payment_amount,source,order_hash,bundle_size)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(
          x['event_id'],x['block_number'],x['block_time'],x['tx_hash'],x['log_index'],x['collection'],
          x.get('event_type','NFT_SOLD'),x.get('token_id'),
          str(x.get('price_wei')) if x.get('price_wei') is not None else None,
          x.get('seller'),x.get('buyer'),json.dumps(x.get('raw',{}),separators=(',',':')),
          x.get('payment_token'),str(x.get('payment_amount')) if x.get('payment_amount') is not None else None,
          x.get('source'),x.get('order_hash'),int(x.get('bundle_size') or 1)
        ))
        self.conn.commit()
    ...
    def market_summary(self,collection,since_ts):
        rows=self.market_window(collection,since_ts)
        native=[r for r in rows if r.get('price_wei') not in (None,'') and int(r.get('bundle_size') or 1)==1]
        unique_sales={r.get('order_hash') or r.get('event_id') for r in rows}
        return {
          'sales_24h':len(unique_sales),
          'native_sales_24h':len(native),
          'volume_eth_24h':sum(int(r['price_wei']) for r in native)/1e18 if native else 0.0,
          'sources':sorted({r.get('source') or 'ONCHAIN' for r in rows}),
        }
    ...
    def observe_shadow(self,candidate,now=None):
        ...
        p=probability_from_score(candidate.get('score'))
        row=self.conn.execute('SELECT * FROM shadow_positions WHERE lower(collection)=lower(?)',(candidate['collection'],)).fetchone()
        if row is None:
            self.conn.execute('''INSERT INTO shadow_positions(collection,first_observed,last_observed,entry_score,entry_action,entry_probability,entry_floor_eth,last_floor_eth,peak_floor_eth)
            VALUES(?,?,?,?,?,?,?,?,?)''',(candidate['collection'],now,now,float(candidate.get('score') or 0),candidate.get('action') or 'WATCH',p,floor,floor,floor))
        else:
    ...
    def record_outcome(self,collection,decision,entry_cost_usd,exit_value_usd,gas_usd=0.0,package_id=None,predicted_score=None,predicted_probability=None):
        ...
        if predicted_probability is None and predicted_score is not None:
            predicted_probability=probability_from_score(predicted_score)
        self.conn.execute('''INSERT INTO outcomes(collection,package_id,decision,entry_cost_usd,exit_value_usd,gas_usd,realized_net_usd,recorded_at,predicted_score,predicted_probability)
        VALUES(?,?,?,?,?,?,?,?,?,?)''',(collection,package_id,decision,float(entry_cost_usd),float(exit_value_usd),float(gas_usd),net,int(time.time()),predicted_score,predicted_probability))
        self.conn.commit()
        return net
    ...
    def calibration_samples(self):
        rows=self.conn.execute("SELECT predicted_probability,realized_net_usd FROM outcomes WHERE predicted_probability IS NOT NULL AND upper(decision) IN ('MINT','BUY')").fetchall()
        out=[{'predicted_probability':r['predicted_probability'],'outcome':1 if float(r['realized_net_usd'] or 0)>0 else 0} for r in rows]
        shadow=self.conn.execute('SELECT * FROM shadow_positions WHERE entry_probability IS NOT NULL AND entry_floor_eth IS NOT NULL AND peak_floor_eth IS NOT NULL AND last_observed-first_observed>=86400').fetchall()
        for r in shadow:
    ...
    def maintenance(self,now=None,observation_days=30,event_days=90):
        now=int(now or time.time())
        last=int(self.get_meta('last_maintenance','0') or 0)
        if now-last<21600:
            return {'ran':False,'integrity':self.integrity_check()}
        self.conn.execute('DELETE FROM observations WHERE observed_at<?',(now-observation_days*86400,))
        self.conn.execute('DELETE FROM mint_events WHERE block_time<?',(now-event_days*86400,))
        self.conn.execute('DELETE FROM market_events WHERE block_time<?',(now-event_days*86400,))
        self.conn.commit()
        try: self.conn.execute('PRAGMA wal_checkpoint(TRUNCATE)')
        except sqlite3.DatabaseError: pass
        self.set_meta('last_maintenance',now)
        return {'ran':True,'integrity':self.integrity_check()}

    def backup(self,backup_dir=None,keep=7):
        backup_dir=backup_dir or os.path.join(os.path.dirname(self.path) or '.','backups')
        os.makedirs(backup_dir,exist_ok=True)
        stamp=time.strftime('%Y%m%d-%H%M%S',time.gmtime())
        target=os.path.join(backup_dir,f'radar-{stamp}.sqlite')
